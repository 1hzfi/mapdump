...
import EventViewing from "./EventViewing";
import EventReplay from "./RouteReplay";
...
const RasterMap = ({ match, history }) => {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const res = await fetch(
        import.meta.env.VITE_API_URL + "mapdump/map-" + match.params.uid,
        {
          credentials: "omit",
          headers,
        }
  ...
  const getComponent = () => {
    ...
    if (showPlayer) {
      return <EventReplay {...props} />;
    }
    return <EventViewing {...props} />;
  };
